...
class Renderer extends EventEmitter {
    ...
    initEvents() {
        ...
        this.wrapper.addEventListener('click', (e) => {
            ...
            // Map x to [0,1] based on visible width
            const x = (e.clientX - rect.left) / rect.width;
            const y = (e.clientY - rect.top) / rect.height;
            this.emit('click', utils.clampToUnit(x), utils.clampToUnit(y));
        });
    ...
    render(audioData) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c;
            // Segment cropping support
            const segmentStart = (_a = this.options.startTime) !== null && _a !== void 0 ? _a : 0;
            const segmentEnd = (_b = this.options.endTime) !== null && _b !== void 0 ? _b : audioData.duration;
            ...
            // Determine the width of the waveform (use segment duration)
            const pixelRatio = this.getPixelRatio();
            ...
            const { scrollWidth, isScrollable, useParentWidth, width } = utils.calculateWaveformLayout({
                ...
                parentWidth,
                fillParent: true, // Force fillParent for segment
                pixelRatio,
            });
            ...
            if (this.options.splitChannels) {
                // Render a waveform for each channel (segment only)
                for (let i = 0; i < audioData.numberOfChannels; i++) {
                    const options = Object.assign(Object.assign({}, this.options), (_c = this.options.splitChannels) === null || _c === void 0 ? void 0 : _c[i]);
                    const segmentData = utils.sliceAudioBuffer(audioData, segmentStart, segmentEnd, i);
                    this.renderChannel([segmentData], options, width, i);
                }
            }
            else {
                // Render a single waveform for the first two channels (segment only)
                const channels = [utils.sliceAudioBuffer(audioData, segmentStart, segmentEnd, 0)];
                if (audioData.numberOfChannels > 1)
                    channels.push(utils.sliceAudioBuffer(audioData, segmentStart, segmentEnd, 1));
