...
class WaveSurfer extends Player<WaveSurferEvents> {
  ...
  /** Jump to a specific time in the audio (in seconds, segment-relative) */
  public setTime(time: number) {
    const { startTime = 0, endTime } = this.options as any
    let t = time
    // Clamp to segment
    if (typeof endTime === 'number' && t > (endTime - startTime)) t = endTime - startTime
    if (t < 0) t = 0
    this.stopAtPosition = null
    super.setTime(t + startTime)
    this.updateProgress(t)
    this.emit('timeupdate', t)
  }

  /** Seek to a ratio of audio as [0..1] (0 = beginning, 1 = end, segment-aware) */
  public seekTo(progress: number) {
    // Always map progress to segment-relative time
    const { startTime = 0, endTime, autoplay } = this.options as any
    let segDuration = this.getDuration()
    if (typeof endTime === 'number') {
      segDuration = endTime - startTime
    }
    // Clamp progress to [0,1]
    const clampedProgress = Math.max(0, Math.min(1, progress))
    const time = segDuration * clampedProgress
    this.setTime(time)
    // If autoplay is true and not playing, resume playback from seeked position
    if (autoplay && !this.isPlaying()) {
      this.play(time)
    }
  }

  /** Update progress (segment-relative) */
  private updateProgress(currentTime = this.getCurrentTime()): number {
    // Make progress relative to segment
    const { startTime = 0, endTime } = this.options as any
    const segTime = Math.max(0, currentTime - startTime)
    let segDuration = this.getDuration()
    if (typeof endTime === 'number') {
      segDuration = endTime - startTime
    }
    this.renderer.renderProgress(segTime / segDuration, this.isPlaying())
    return segTime
  }
  ...
  private initPlayerEvents() {
    ...
    this.mediaSubscriptions.push(
      this.onMediaEvent('timeupdate', () => {
        ...
        // If playback is before the segment, jump to startTime
        if (current < startTime) {
          this.setTime(0) // segment-relative 0
          current = startTime
        }
        // Clamp playback to segment end only if playing and past end
        if (typeof endTime === 'number' && this.isPlaying() && current >= endTime) {
          this.pause()
          this.setTime(endTime - startTime)
          return
        }
        const segTime = this.updateProgress(current)
        this.emit('timeupdate', segTime)
      }),
    ...
    this.subscriptions.push(
      ...
      this.renderer.on('click', (relativeX, relativeY) => {
        if (this.options.interact) {
          // Map click to segment-relative seek
  ...
  /** Start playing the audio (segment-aware) */
  public async play(start?: number, end?: number): Promise<void> {
    const { startTime = 0, endTime } = this.options as any
    // Always play the segment
    const playStart = start != null ? start : undefined
    const playEnd = end != null ? end : (typeof endTime === 'number' ? endTime - startTime : undefined)
    // Only seek if playStart is provided and different from current segment-relative time
    if (playStart != null) {
      const currentSegTime = this.getCurrentTime() - startTime
      if (Math.abs(currentSegTime - playStart) > 0.01) {
        this.setTime(playStart)
      }
    }
    const playResult = await super.play()
    if (playEnd != null) {
      if (this.media instanceof WebAudioPlayer) {
        this.media.stopAt(playEnd + startTime)
      } else {
        this.stopAtPosition = playEnd + startTime
      }
    }
    return playResult
  }
