...
class Renderer extends EventEmitter<RendererEvents> {
  private options: WaveSurferOptions & { startTime?: number, endTime?: number }
  ...
  async render(audioData: AudioBuffer) {
    ...
    const segmentStart = this.options.startTime ?? 0;
    const segmentEnd = this.options.endTime ?? audioData.duration;
    ...
    if (this.options.splitChannels) {
      ...
      if (audioData.numberOfChannels > 1) channels.push(utils.sliceAudioBuffer(audioData, segmentStart, segmentEnd, 1))
