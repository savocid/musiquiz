// Helper: slice an AudioBuffer to a segment (returns Float32Array)
export function sliceAudioBuffer(audioBuffer: AudioBuffer, start: number, end: number, channel: number): Float32Array {
  const sampleRate = audioBuffer.sampleRate;
  const startSample = Math.floor(start * sampleRate);
  const endSample = Math.floor(end * sampleRate);
  return audioBuffer.getChannelData(channel).slice(startSample, endSample);
}
